...
export class PersistenceService {
    ...
    insertOrUpdateMyContacts(contacts, cbOnSuccess, cbOnFailure) {
        ...
        try {
            ...
            for (var j = 0; j < contacts.length; j++) {
                var phoneInMyContactsNLS = self.utilsService.phoneSanitize(contacts[j].phone);
                contacts[j].phone = phoneInMyContactsNLS;
                var iof = existsInLS.indexOf(phoneInMyContactsNLS);
                if (iof == -1) {
                    mycontacts.push(contacts[j]);
                } else {
                    mycontacts[iof] = contacts[j];
